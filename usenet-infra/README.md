...
For checkout-loss recovery, start with the [repository-wide secrets inventory](docs/secrets-recovery.md)
and run `just --no-dotenv secrets-check` from the repository root. This checks metadata only;
