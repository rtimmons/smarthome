...
- RGBW scenes select one physical endpoint explicitly: use the `_white` entity (Switch Multilevel endpoint 5) for white brightness, and use the aggregate RGBW entity (endpoint 1) once for color or all-channel off. Never auto-expand a target to both endpoints. Repo-owned Z-Wave JS priority configs under `new-hass-configs/zwave-device-configs/` disable background auto-refresh for the installed RGBW controller models; deploy them with `just zwave-deploy-device-configs`.
