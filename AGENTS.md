...
- Operational lighting must use `script.fast_scene_<scene_id>`, never native `scene.turn_on`; restart-mode wrappers and dispatcher make the newest request authoritative while preserving health filtering, serial 250-ms-paced Z-Wave submissions, error isolation, skipped-target reporting, and a restart-cancellable mismatch-only convergence pass. Non-Z-Wave and Z-Wave branches start together. Compatible dimmer hardware uses isolated CC38 `zwave_js.set_value` with `wait_for_result: false` and a zero-second transition. Multicast is currently forbidden for every device because repeated normal-operation failures invalidated the former Minoston MP22ZD allowlist. See `docs/operations/zwave-scene-ops.md`.
