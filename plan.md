...
TV-root migration and selected media-002 copy verification. The Downloads task
released Git/NAS ownership to this task. Native source `a4ec62c`, telemetry
`dfef005`, and the preflight handoff `58c1eec` are published on `usenet` at
...
legacy title actions and the full-width Downloads panel. media-002 — Knightfall,
...
| Selective copies | Native and legacy OliveTin dashboard `http://192.168.1.66:1337/` refreshes automatically. Verified copies go beneath `/share/FromDrobo/Movies/video`; `.staging` stays outside Plex sources. `media-003` (13.5 GiB) copied and indexed successfully. Keep 100 GiB NAS reserve. |
