...
- **HA policy matrix:** `topology, preset, status, and retained convenience policies are explicit`, `zones omit an unavailable portable room when no reachable group claims it`, and `zones fail closed when a reachable group still claims an unavailable room` in the same suite.
...
| --- | --- | --- | --- | --- | --- | --- |
| `GET /sonos/zones` | `GET /sonos/zones` | Active: Dashboard topology polling and membership controls | `200`; reachable legacy zone array, coordinator first and other members sorted; freshness headers; an unclaimed unavailable portable room is omitted and named in `X-Sonos-Unavailable-Rooms` | Transport-unknown topology and a reachable group that still claims an unavailable room fail closed rather than returning synthetic membership; retained | HA success/failure/policy, Grid contract, backend read isolation | Live HA/node/UI/Sonos-app parity pending |
...
| `POST /sonos-intents/group-all` | `POST /intents/sonos/group-all`; direct Sonos API alias `/sonos-intents/group-all` is also retained | Active: Dashboard group-all | `202 {intent}`; zero writes if satisfied; otherwise each missing reachable room receives one ordered, isolated join after an optional target unjoin, so one room failure yields a partial result without blocking later rooms | `400 invalid_request` for missing/empty/duplicate/mismatched list; `404` invalid room; `503` non-live/unavailable target; unavailable or failed members are reported in the partial result; retained | HA success/failure/policy, Grid contract/alias, action/queue suites | Live singleton/follower/double-tap and partial-member matrix pending |
...
| Complete state projection, requested-member/coordinator ownership, all coordinators, multi-group/singleton shapes, malformed topology, and freshness headers | `home-assistant-sonos-state.spec.ts`; `zones and room-state headers and bodies each derive from one immutable snapshot`; portable-room omission and claimed-member fail-closed route regressions | Automated complete | Final clean-SHA rerun and live eight-room/UI/Sonos-app comparison pending |
