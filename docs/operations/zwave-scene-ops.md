...
button/webhook/dashboard
  ...
  -> global queued Z-Wave gate, one submission paced every 500 ms
...
- A device with `fastSceneForceOnDispatch: true` is the narrow exception to mismatch-only dispatch: turn-on requests are sent during both the initial and convergence passes even when Home Assistant already reports a match; turn-off remains mismatch-only. Node 7 (`living_curtains`) uses this because its aggregate RGBW entity remained optimistically `on` while the physical strip was dark. Do not broaden this workaround without a reproduced physical/cache mismatch; it intentionally adds one retry to the paced Z-Wave queue.
...
- Empty Z-Wave batches are skipped. The global queued gate places the 500 ms spacing after each actual submission, so concurrent room requests cannot collapse back into an unsafe burst.
...
- `DEFAULT_MAX_ZWAVE_CALLS_PER_STEP` is one. After removing the failed Minoston multicast allowlist, a live two-call Living High/Off replay still produced a same-burst decode/invalid-payload cluster on nodes 4 and 6, so submissions were serialized on 2026-08-31. Two more Living replays on 2026-09-01 produced controller jams, invalid frames, and node-6/node-10 timeouts at 250 ms even with one global gate; `DEFAULT_ZWAVE_BATCH_DELAY_MS` is therefore 500 ms across all concurrent scene requests.
...
The two bindings were corrected on 2026-09-01: both canonical `_white` entities now point to endpoint 5, and their endpoint 2-4 entities are explicitly named red/green/blue and disabled. This is useful registry hygiene, but it does not make the individual endpoints suitable for scene control. Later physical testing showed all Kitchen and Living `_white` entities reporting `on` while their strips remained dark; paced aggregate commands energized all eight strips immediately.

After any RGBW change, verify all three layers:

1. The live entity registry maps `_white` to `-38-5-currentValue` and keeps endpoints 2-4 disabled; treat these as diagnostic entities only.
2. Every generated white, color, and off scene contains the aggregate entity exactly once and contains no `_white` entity. White targets include explicit `[0, 0, 0, level]` RGBW data.
3. Physical strips match the requested output; Home Assistant endpoint state alone is not accepted as proof.
4. The Z-Wave JS startup log reports `User-provided device config loaded` for the RGBW nodes after `just zwave-deploy-device-configs`.
5. The subsequent persistent log contains no new `Multilevel Sensor CC values may be stale, refreshing` sequence for those nodes; the startup message alone is not sufficient evidence.
...
- A 100 ms pacing trial made Living High faster but produced four invalid payloads and a new node-3 timeout, so 250 ms was retained at that stage. Running the independent non-Z-Wave branch concurrently still provides immediate visible response while isolated Z-Wave calls remain bounded.
- Later 2026-09-01 Living Off/High verification showed that 250 ms also remained below the reliable floor under a full room transition: one run produced 16 invalid payloads and two controller-jammed events; a narrowed retry still produced four invalid payloads plus node-6 and node-10 timeouts. Production pacing was increased to 500 ms before commit.
- The final 500 ms Living Off/High replay settled all reachable targets without a controller jam or node timeout and left the curtain at full aggregate white. One invalid frame remained at 19:53:24 while node 3 (`living_behindtv`) was in an automatic endpoint-sensor refresh that began during the scene despite its loaded priority config; that residual background-refresh fault is distinct from scene submission pacing.
...
- The same remediation corrected node 7 and node 10 `_white` registry bindings from endpoints 4 and 2 to endpoint 5, disabled and named the individual color endpoints, removed automatic RGBW pair expansion, and changed off scenes to use one aggregate command. Repo-owned priority configs loaded successfully for every installed ZEN31/FGRGBW-442 node and disable the unused-sensor auto-refresh path. Later live testing proved endpoint 5 was still not a reliable operational control path: all Kitchen and Living `_white` entities reported `on` while the strips were dark, while one paced aggregate command per controller turned on all eight strips. Operational white scenes now use aggregate RGBW `[0, 0, 0, level]` commands; individual endpoints remain diagnostic-only.
- On 2026-09-01, the Living curtain strip later remained physically dark while its aggregate entity still reported the exact Living High state from the prior exercise. An identical turn-on did not produce a state transition, while a forced aggregate Off/On produced fresh live transitions. Node 7 is now the only `fastSceneForceOnDispatch` target, so both the initial and existing convergence passes transmit its requested aggregate on state instead of trusting the optimistic cache. The first broader trial also forced off and coincided with node-4 nonce errors, invalid frames, and two controller-jammed events; force dispatch was therefore restricted to on requests before commit.
