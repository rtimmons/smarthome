...
- A live, individually unavailable portable room is distinct from transport-unknown topology. If no reachable group's authoritative `group_members` list claims that room, `/sonos/zones` may return the complete reachable groups, omit the unavailable room, and name it in `X-Sonos-Unavailable-Rooms`; the Dashboard keeps those groups usable and shows a concise unavailable-room banner. If any reachable group still claims the unavailable room, the whole affected topology remains fail-closed.
