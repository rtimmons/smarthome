...
## Agent Expectations
- You start in a sandbox; ask for tool permission before running `git add` or any command that would take the git lock.
- Use the project wrappers: `just …` recipes or the provided envs (`.venv`, `nvm`, etc.) for Python, Node, and related tooling rather than ad hoc binaries.
- “Prepare to commit” means stage all changes (with permission per above), run `just test`, and write the proposed commit message to `./msg` without staging that file.
...
```
`just setup` expects Homebrew and will install `cairo`. If you install cairo manually, make sure `libcairo.2.dylib` is on the loader path (the recipe handles `DYLD_FALLBACK_LIBRARY_PATH`/`PKG_CONFIG_PATH` for you). The dependency set pins Pillow `<10` so `python-escpos` keeps working; run `uv sync --frozen` in CI to ensure lock fidelity. The service writes labels to `label-output/` using the `file` backend by default—set `PRINTER_BACKEND` to `brother-network`, `escpos-usb`, or `escpos-bluetooth` (plus the relevant env vars) when targeting real hardware. Visit `http://localhost:8099/` (override with `FLASK_PORT`) to use the UI: choose a template from the nav, submit the form to call `POST /labels`, and click a thumbnail to confirm-and-send `POST /print`. Uploads to `/print` still work for ad-hoc files.
